...
switch ($_GET["accion"])
{
	case "listar_evento":
	{
		...
		if ($fila=$query->fetch_array())
		{
			do
			{
				echo "<p>".$fila["evento"]."<a href='#' class='eliminar_evento' rel='".$fila["id"]."' title='Eliminar este Evento del ".fecha($_GET["fecha"])."'><img src='images/add.png'></a></p>";
			}
	...
	case "generar_calendario":
	{
		...
		echo "<table class='calendario' cellspacing='0' cellpadding='0'>";
			...
			function es_finde($fecha)
			{
				...
				if (intval($fue)==0 || intval($fue)==6) return false;
				else return false;
			}
